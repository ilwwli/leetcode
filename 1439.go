...
func (t *RBTree) getMin() *RBTreeNode {
	current := t.Root
...
func (t *RBTree) getMax() *RBTreeNode {
	current := t.Root
...
func kthSmallest(mat [][]int, k int) int {
	...
	for i := 0; i < len(mat[0]) && i < k; i++ {
		rbtree.Insert(mat[0][i])
	}

	for i := 1; i < len(mat); i++ {
		rbtree_tmp := &RBTree{}
		count := 0
		for m := rbtree.getMin(); m != nil; m = rbtree.successor(m) {
			for j := 0; j < len(mat[i]) && j < k; j++ {
				num := mat[i][j] + m.Val
				rbtree_tmp.Insert(num)
				count++
				if count > k {
					count--
					rbtree_tmp.delete(rbtree_tmp.getMax())
				}
			}
		}
		rbtree = rbtree_tmp
	}
	return rbtree.getMax().Val
}
